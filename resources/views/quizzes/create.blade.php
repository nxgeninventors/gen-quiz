<x-admin.wrapper>
    ...
    <div>
        <x-admin.breadcrumb href="{{route('tests.index')}}" title="{{ __('Create Test') }}">{{ __('<< Back to all tests') }}</x-admin.breadcrumb>
        <x-admin.form.errors />
    </div>
        ...
        @csrf
            ...
            <div class="py-2">
                ...
                <x-select name="quiz_type" class="{{$errors->has('quiz_type') ? 'border-red-400' : ''}}">
                    ...
                    <option {{ old('quiz_type') == $quiz_type ? 'selected' : '' }} value="{{ $quiz_type }}">{{ $quiz_type }}</option>
                    @endforeach
                </x-select>
            </div>
            ...
            <div class="py-2">
                ...
                <x-select name="quiz_category_id" class="{{$errors->has('quiz_category_id') ? 'border-red-400' : ''}}">
                    ...
                    <option {{ old('quiz_category_id') == $quizCategory->id ? 'selected' : '' }}  value="{{ $quizCategory->id }}">{{ $quizCategory->name }}</option>
                    @endforeach
                </x-select>
            </div>
